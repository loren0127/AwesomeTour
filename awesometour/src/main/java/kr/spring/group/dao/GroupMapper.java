...
public interface GroupMapper {
	//수정용 주석
	@Select("SELECT COUNT(*) FROM group_table WHERE g_isSearch = 0 AND g_num != -1")
	public int selectRowCount(Map<String,Object> map);
	public int selectRowCountFilter(Map<String,Object> map);
	@Select("SELECT * FROM (SELECT a.*, rownum rnum FROM (SELECT * FROM group_table WHERE g_isSearch = 0  AND g_num != -1ORDER BY g_reg_date DESC)a )  WHERE rnum >= #{start} AND rnum <= #{end} ")
