					...
					.ajax({
						...
						success : function(data) {
							...
							if (data.result == 'success') {
								...
								for (var i = 0; i < 3; i++) {
									pagingAppend += '<div class="col-3"><div class="hobby_small rounded" style="height:30px; width:100%;text-align:center;background-color:gray; margin: auto;"><h6>'+ array[i]+ '</h6></div></div>';
								...
								if (item.ro_sub == 'h') {
									pagingAppend += '<br>';
									pagingAppend += '<a href="../accomDetail/accomDetail_hotel.do?im_ac_num='+ item.acc_num+ '&check_in=${map.check_in}&check_out=${map.check_out}&people_count=${map.people_count}&search=${map.search}">자세히보기</a>';
								}

								if (item.ro_sub == 'p') {
									pagingAppend += '<br>';
									pagingAppend += '<a href="../accomDetail/accomDetail_private.do?im_ac_num='+ item.acc_num+ '&check_in=${map.check_in}&check_out=${map.check_out}&people_count=${map.people_count}&search=${map.search}">자세히보기</a>';
								}
...
<form id="se_form">
	<div>
		...
		<div class="sticky-top"
			...
			<!-- 검색 시작 -->
			<div class="form-row" style="width: 1200px; margin-left: 200px;">
				<div>
		...
		<div class="container">
			...
			<div class="row" style="margin-top: 50px;">
				<div id="map" class="col-3">
					<img src="../resources/images/지도.jpg" id="opener2"><br>
					...
					<c:forEach var="accom" items="${list}">
						<div class="row" style="margin-bottom: 20px;">
							<div class="col-10"
								...
								<div class="service">
									<div class="row">
										<input type="hidden" value="${accom.se_name}"> <span>${accom.acc_theme}</span>
										<c:set var="array" value="${fn:split(accom.se_name,',')}" />
										<!-- 서비스를 배열형태로 반환하여 실행함 -->
										<c:forEach var="hobby" items="${array}" begin="0" end="2">
											<div class="col-3">
												<div class="hobby_small rounded"
													style="height: 30px; width: max-content; text-align: center; background-color: gray; margin: auto;">
													<h6>${hobby}</h6>
												</div>
											</div>
										</c:forEach>
							...
							<div class="col-2"
								style="height: 150px; border: 1px solid #e5e5e5;" id="col-3">
								<span>1박요금</span><br>
