			...
			<div class="inbox_chat">
				<c:if test="${!empty user_email}">
					...
					<div class="tab-content">
						...
						<!-- 보낸 쪽지 -->
						<div id="send_message_chat" class="container tab-pane fade <c:if test="${selected eq 'receiveMessageList'}">active show</c:if>">
							<div class="chat_list">
								<div class="chat_people">
									<div class="chat_ib">
										<h5>
								...
								<c:forEach var="receiveMessageCommand" items="${receiveMessageList}">
									<div class="chat_list">
										<div class="chat_people">
											...
											<div class="chat_ib">
												<h5>
													<a href="selectMessageReceiveDetail.do?message_num=${receiveMessageCommand.message_num}&checked=group">${receiveMessageCommand.message_title}</a>
													<br><br>
													<span class="chat_date">${receiveMessageCommand.message_reg_date}</span>
												</h5>
												<p>${receiveMessageCommand.message_sender}</p>
											</div>
										</div>
									</div>
								</c:forEach>
								<div align="center">${receivePagingHtml}</div>
							</c:if>
						</div>
						
						<!-- 받은 쪽지 -->
						<div id="receive_message_chat" class="container tab-pane fade <c:if test="${selected eq 'sendMessageList'}">active show</c:if>">
							<div class="chat_list">
								<div class="chat_people">
									<div class="chat_ib">
										<h5>
							...
							<c:if test="${receiveMessageCount>0}">
								<c:forEach var="sendeMessageCommand" items="${sendMessageList}">
									<div class="chat_list">
										<div class="chat_people">
											<div class="chat_img">
												<img src="https://ptetutorials.com/images/user-profile.png" alt="sunil">
											...
											<div class="chat_ib">
												<h5>
													<a href="selectMessageReceiveDetail.do?message_num=${sendeMessageCommand.message_num}&checked=group">${sendeMessageCommand.message_title}</a>
													<br><br>
													<span class="chat_date">${sendeMessageCommand.message_reg_date}</span>
												</h5>
												<p>${sendeMessageCommand.message_receiver}</p>
											</div>
										</div>
									</div>
								</c:forEach>
